      ...
      <div class="grid grid-cols-7 gap-4">
        <div class="col-span-3 text-center">
          <span class="text-xl text-black"
            >Request <span class="font-bold">Instant Help</span></span
          >
          <div class="px-5 py-5 bg-white rounded-md mt-3 leftbox">
            <textarea
              ...
              "
              rows="3"
              placeholder="How can we help? Describe the problem..."
            ></textarea>
            <p class="text-left text-md mt-3 text-black">
              Use
              <a
                href="http://www.loom.com"
                target="_blank"
                class="text-blue-600"
                >Loom.com</a
              >
              to easily screen record your question.
            </p>
            <input
              v-model="loom"
              type="text"
              placeholder="Paste a link to your loom screen record...(optional)"
              class="
                mt-1
                px-1
                py-1
            ...
            <p class="mt-2 text-left">
              ...
              <label for="checkbox" class="text-black">
                Make this post to reach more experts.</label
              >
            </p>
        ...
        <div class="col-span-4 text-center">
          <span class="text-xl text-black"
            >Expert <span class="font-bold">Responses</span></span
          >
          <div
            v-for="item in responses"
            :key="item.id"
            class="px-5 py-3 bg-white rounded-md mt-3 rightbox"
          >
            <div
              class="
                text-left text-md text-black
                flex
                items-center
                justify-between
              "
            >
              <div>
                ...
                <span class="text-sm pl-2">{{
                  getDiffTime(item.question.created_at)
                }}</span>
              </div>
              <div>
                <a
                  class="text-sm text-blue-500 underline"
                  :href="item.question.loom"
                  target="_blank"
                  >Loom link</a
                >
                <a
                  class="text-sm pl-3 underline"
                  :href="baseURL + '/api/download/' + item.question.id"
                  target="_blank"
                  >Attached File</a
                >
              </div>
              <div class="flex">
                <span
                  ...
                  :class="
                    item.question.visibility == 'true'
                      ? 'text-green-500'
                      : 'text-red-500'
                  "
                  >{{
                    item.question.visibility == "true" ? "Public" : "Private"
                  }}</span
                >
                    ...
                    <div class="py-2 flex justify-between items-center">
                      ...
                      <button
                        class="
                          ...
                          border-yellow-300
                        "
                        @click="submitReply(item.question.id)"
                      >
                        Reply
                      </button>
                    </div>
                    ...
                    >
                      <div class="flex">
                        ...
                        <div>
                          <p class="text-left">
                            <span
                              class="text-sm font-bold text-black text-left"
                            >
                      ...
                      <div class="flex">
                        ...
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          class="h-6 w-6 mx-1 cursor-pointer"
                          viewBox="0 0 24 24"
                          fill="currentColor"
                        >
                          <path
...
export default {
  ...
  methods: {
    ...
    async submitReply(id) {
      ...
      const { data } = await axios.post(baseURL + "/api/submitResponse", {
        question_id: id,
        user_id: user_id,
    ...
    getDiffTime(created_at) {
      ...
      return (
        getTimeDiff(st, today).value +
        " " +
        getTimeDiff(st, today).suffix +
        " ago"
      );
    },
    async fetchResponses() {
      const { data } = await axios.get(baseURL + "/api/fetch_response");
      if (data.status) {
...
div.rightbox {
  box-shadow: 0 2.8px 2.2px rgba(0, 0, 0, 0.034),
    0 6.7px 5.3px rgba(0, 0, 0, 0.048), 0 12.5px 7px rgba(0, 0, 0, 0.06);
}
